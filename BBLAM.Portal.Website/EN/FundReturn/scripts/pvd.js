$(function () {
    ...
    $(document).on('click', '.btn-upload', function (e) {
        ...
        BootstrapDialog.show({
            title: 'Upload PVD Return Text File',
            closable: false,
            message: function (dialogRef) {
                ...
                $file.kendoUpload({
                    ...
                    async: {
                        ...
                        saveUrl: rootapi + '/api/pvd/return/file-save',
                        removeUrl: rootapi + '/api/pvd/file-remove',
                        autoUpload: false,
                    },
                    //files: files,
                    ...
                    success: function (e) {
                        ...
                        if (e.operation == 'upload') {
                            if (e.response && e.files[0]) {
                                var uid = e.files[0].uid;
                                $('li.k-file[data-uid="' + uid + '"]').remove();
                                $.App.ui.dialog.success('PVD Return was uploaded successfully.', function () {
                                    setupGrid();
                                    dialog.close();
                                });
                            } else {
                                $.App.ui.dialog.alert('Failed to upload PVD Return, please revise file and try again.', function () {
                                    dialog.close();
                                });
                            }
                        }
            ...
            buttons: [{
                ...
                action: function (dialogRef) {
                    ...
                    if (upload) {
                        ...
                        if (totalFiles > 0) {
                            $.App.ui.dialog.confirm('Are you sure you want to upload \'PVD Return Text File\' ?', function (result) {
                                if (result) {
                                    dialog = dialogRef;
                            ...
                            $.App.ui.dialog.alert('Please select at least one \'PVD Return Text File\' file before upload.');
                        }
                    }
                }
            }]
    ...
    $(document).on('click', '.open-edit-dialog, .btn-add', function (e) {
        ...
        BootstrapDialog.show({
            title: obj.port_code ? 'Update PVD Return of ' + obj.port_code : 'Add new PVD Return',
            closable: false,
            message: function (dialogRef) {
                ...
                $form
                    ...
                    .on('success.form.fv', function (e) {
                        ...
                        var $form = $(e.target);
                        var msg = 'Are you sure you want to save this PVD return ?';
                        $.App.ui.dialog.confirm(msg, function (result) {
                            if (result) {
                                ...
                                $.ajax({
                                    ...
                                    url: rootapi + '/api/pvd/return/save',
                                    contentType: 'application/json; charset=utf-8',
                                    dataType: 'json',
                                    success: function (data, status, xhr) {
                                        $.App.ui.dialog.success('The PVD return was saved successfully.', function () {
                                            dialogRef.close();
                                            setupGrid();
                                        });
                                    },
    ...
    $(document).on('click', '.delete-data', function (e) {
        ...
        var msg = 'Are you sure you want to delete this PVD return ?';
        $.App.ui.dialog.confirm(msg, function (result) {
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    data: JSON.stringify(req),
                    url: rootapi + '/api/pvd/return/delete',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (data, status, xhr) {
                        $.App.ui.dialog.success('The PVD return was deleted successfully.', function () {
                            setupGrid();
                        });
                    },
                    error: function (xhr, status, error) {
    ...
    $grid.kendoGrid($.extend(true, options, {
        dataSource: {
            ...
            transport: {
                read: {
                    contentType: 'application/json; charset=utf-8',
                    url: rootapi + '/api/pvd/return/list',
                },
        ...
        excel: {
            fileName: kendoUtility.excelExport.createFileName('pvd_return', asof),
        },
