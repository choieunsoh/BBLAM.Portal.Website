...
$(function () {
    ...
    $(document).on('click', '.btn-upload', function (e) {
        ...
        BootstrapDialog.show({
            title: 'Upload Equity Limit CSV File',
            closable: false,
            message: function (dialogRef) {
                ...
                $file.kendoUpload({
                    ...
                    async: {
                        ...
                        saveUrl: rootapi + '/api/limit/equity/file-save',
                    ...
                    success: function (e) {
                        ...
                        if (e.operation == 'upload') {
                            if (e.response && e.files[0]) {
                                var uid = e.files[0].uid;
                                $('li.k-file[data-uid="' + uid + '"]').remove();
                                $.App.ui.dialog.success('Equity Limit was uploaded successfully.', function () {
                                    setupGrid();
                                    dialog.close();
                                });
                            } else {
                                $.App.ui.dialog.alert('Failed to upload Equity Limit, please revise file and try again.', function () {
                                    dialog.close();
                                });
                            }
                        }
            ...
            buttons: [{
                ...
                action: function (dialogRef) {
                    ...
                    if (upload) {
                        ...
                        if (totalFiles > 0) {
                            $.App.ui.dialog.confirm('Are you sure you want to upload \'Equity Limit CSV File\' ?', function (result) {
                                if (result) {
                                    dialog = dialogRef;
                            ...
                            $.App.ui.dialog.alert('Please select at least one \'Equity Limit CSV File\' file before upload.');
                        }
                    }
                }
            }]
    ...
    $(document).on('click', '.open-edit-dialog, .btn-add', function (e) {
        ...
        BootstrapDialog.show({
            title: obj.fund_code ? 'Update Equity Limit of ' + obj.fund_code : 'Add new Equity Limit',
            closable: false,
            message: function (dialogRef) {
                ...
                $form
                    ...
                    .formValidation({
                        ...
                        fields: {
                            ...
                            dialogMinLimit: {
                                selector: '[data-field=min_limit]',
                                validators: {
                                    notEmpty: {
                                        message: 'Please enter equity min limit.',
                                    },
                                    between: {
                                        min: 0,
                                        max: 'dialogMaxLimit',
                                        message: 'The equity min limit must be between %s and %s.',
                                    },
                                },
                            },
                            dialogMaxLimit: {
                                selector: '[data-field=max_limit]',
                                validators: {
                                    notEmpty: {
                                        message: 'Please enter equity max limit.',
                                    },
                                    between: {
                                        min: 0,
                                        max: 1,
                                        message: 'The equity max limit must be between %s and %s.',
                                    },
                                },
                            },
                        },
                    ...
                    // Revalidate
                    .on('keyup', '[data-field=max_limit]', function (e) {
                        $form.formValidation('revalidateField', 'dialogMinLimit');
                    })
                    .on('success.form.fv', function (e) {
                        ...
                        var $form = $(e.target);
                        var msg = 'Are you sure you want to save this equity limit ?';
                        $.App.ui.dialog.confirm(msg, function (result) {
                            if (result) {
                                ...
                                $.ajax({
                                    ...
                                    url: rootapi + '/api/limit/equity/save',
                                    contentType: 'application/json; charset=utf-8',
                                    dataType: 'json',
                                    success: function (data, status, xhr) {
                                        $.App.ui.dialog.success('The equity limit was saved successfully.', function () {
                                            dialogRef.close();
                                            setupGrid();
                                        });
                                    },
    ...
    $(document).on('click', '.open-log-dialog', function (e) {
        ...
        $logDialog = BootstrapDialog.show({
            title: 'Equity Limit of ' + obj.fund_code,
                ...
                $grid.kendoGrid($.extend(true, options, {
                    dataSource: {
                        ...
                        transport: {
                            read: {
                                contentType: 'application/json; charset=utf-8',
                                url: rootapi + '/api/limit/equity/list',
                            },
                    ...
                    excel: {
                        fileName: kendoUtility.excelExport.createFileName(obj.fund_code + '_equity_limit', moment()),
                    },
                    showTooltip: true,
                    selectable: false,
                    scrollable: true,
                    ...
                    columns: [{
                        ...
                        field: 'min_limit',
                        title: 'Equity Min Limit',
                        width: 100,
                    }, {
                        attributes: { 'class': 'text-right' },
                        field: 'max_limit',
                        title: 'Equity Max Limit',
                        width: 100,
                    }, {
                        attributes: { 'class': 'text-right' },
                        field: 'var_limit',
    ...
    $(document).on('click', '.delete-limit', function (e) {
        ...
        var msg = 'Are you sure you want to delete this equity limit ?';
        $.App.ui.dialog.confirm(msg, function (result) {
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    data: JSON.stringify(req),
                    url: rootapi + '/api/limit/equity/delete',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (data, status, xhr) {
                        if ($logDialog) {
                            $.App.ui.dialog.success('The equity limit was deleted successfully.', function () {
                                $logDialog.close();
                                setupGrid();
                            });
                        }
    ...
    $grid.kendoGrid($.extend(true, options, {
        dataSource: {
            ...
            transport: {
                read: {
                    contentType: 'application/json; charset=utf-8',
                    url: rootapi + '/api/limit/equity/latest',
                },
                parameterMap: function (data, type) {
                    return {
                        asof: asof,
