...
(function () {
    ...
    var _notshift = ['04/13', '04/14', '04/15', '07/01', '12/31'];
    var _sundayshift = ['04/14', '04/15', '01/01'];
    var _holidays = {
        'M': {//Month, Day
            '01/01': { 'name': "New Year's Day" },
            '04/06': { 'name': "Chakri Day" },
            '04/13': { 'name': "Songkran Festival" },
            '04/14': { 'name': "Songkran Festival" },
            '04/15': { 'name': "Songkran Festival" },
            '05/01': { 'name': "National Labour Day" },
            '05/05': { 'name': "Coronation Day", 'max': 2016 },
            '07/01': { 'name': "Mid Year Closing Day" },
            '07/28': { 'name': "H.M. King Maha Vajiralongkorn's Birthday", 'min': 2017 },
            '08/12': { 'name': "H.M. the Queen's Birthday" },
            '10/13': { 'name': "H.M. the late King Bhumibol Adulyadej Memorial Day", 'min': 2017 },
            '10/23': { 'name': "Chulalongkorn Day" },
            '12/05': { 'name': "H.M. the late King Bhumibol Adulyadej's Birthday" },
            '12/10': { 'name': "Constitution Day" },
            '12/31': { 'name': "New Year's Eve" }
        }
    };
    ___validHoliday = function (d) {
        var y = d.years();
        var x = _holidays['M'][d.format('MM/DD')];
        if (!x) {
            return false;
        }
        if (x.min && y < x.min) {
    ...
    moment.fn.holiday = function () {
        if (this.isoWeekday() === 1 && !_holidays['M'][this.format('MM/DD')]) {
            var d1 = this.clone().add('d', -1);
            var d2 = this.clone().add('d', -2);
            if (_sundayshift.indexOf(d1.format('MM/DD')) !== -1 || (_notshift.indexOf(d1.format('MM/DD')) === -1 && _notshift.indexOf(d2.format('MM/DD'))) === -1) {
                if (___validHoliday(d1) || ___validHoliday(d2)) {
                    var x = (_holidays['M'][d1.format('MM/DD')] && _holidays['M'][d1.format('MM/DD')].name) ||
                        (_holidays['M'][d2.format('MM/DD')] && _holidays['M'][d2.format('MM/DD')].name);
                    if (x) {
                        return 'Substitution for ' + x;
                    }
                }
            }
        }
        ...
        if (___validHoliday(this)) {
            return (_holidays['M'][this.format('MM/DD')] && _holidays['M'][this.format('MM/DD')].name) ||
                (_holidays['W'][this.format('M/' + (memorial || diff) + '/d')] && _holidays['W'][this.format('M/' + (memorial || diff) + '/d')].name);
        } else {
            return '';
        }
    };
    moment.fn.holidayText = function () {
        try {
            return this.holiday();
        } catch (err) {
            return '';
    ...
    moment.fn.isHoliday = function () {
        ...
        if (this.isoWeekday() === 6 || this.isoWeekday() === 7)
            return true;
        ...
        try {
            if (___validHoliday(this)) {
                var x = (_holidays['M'][this.format('MM/DD')] && _holidays['M'][this.format('MM/DD')].name) ||
                    (_holidays['W'][this.format('M/' + (memorial || diff) + '/d')] && _holidays['W'][this.format('M/' + (memorial || diff) + '/d')].name);
                return true;
            } else {
                return false;
   ...
   */
    ...
    moment.fn.isEndOfMonth = function () {
        return this.clone().add(1, 'days').date() === 1;
    };
